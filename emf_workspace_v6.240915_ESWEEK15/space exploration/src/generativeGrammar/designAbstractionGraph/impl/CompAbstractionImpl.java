...
public class CompAbstractionImpl extends EObjectImpl implements CompAbstraction {
	...
	public List<String> getUnconnectedPorts(String portType, String busType) {
		...
		System.out.println("Unconnected Ports: " + finalunconnectedPorts.toString() + "\n");
		return finalunconnectedPorts;
	}
...
//			return;
//		}
	} // End Method setPortConnections

	public boolean IsEquivalent(CompAbstraction comp, boolean strict) {
		boolean equi = true;

		System.out.println("");
		System.out.println("Equvalence Check: Checking equivalence for components: " + this.getInstanceName() + " & " + comp.getInstanceName() + " Strictness: " + strict);
		String reason = "";
		if (comp.getIS_BUS() && this.getIS_BUS()) {
			if (comp.getMirMasterPorts().size() == this.getMirMasterPorts().size() &&
					...
					comp.getInstanceName().equals(this.getInstanceName())  // Munish WARNING: This makes it specific. For generalization it this check should be avoided.
					) {
			equi = true;
			System.out.println("Equivalence Check: Component mapping is Equivalent (Bus equivalent): " + this.getInstanceName() + "=="
		...
		if (!equi) {
			System.out.println("Equivalence Check: Component mapping is not Equivalent: " + this.getInstanceName() + "!="
					+ comp.getInstanceName());
			System.out.println("Equivalence Check: Mismatch was found (Library mismatch):" + reason);
			return equi;
		}
		...
		if (!comp.getIS_BUS()) {
			...
			System.out.println("Equivalence Check: Equivalence found between :" + this.getInstanceName() + " and " + comp.getInstanceName());
